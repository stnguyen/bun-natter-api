...
async function migrateToLatest(database: Database) {
  ...
  const lastMigration = (await database
    .query(
      `
  ...
  const migrationFiles = (await readdir(MIGRATION_DIR)).sort();

  // Only get `.up.sql` files
  const upMigrationFiles = migrationFiles.filter((file) =>
    file.endsWith(".up.sql")
  );
  const upMigrationNames = upMigrationFiles.map((file) =>
    file.replace(".up.sql", "")
  );

  // Get the last applied migration index
  const lastMigrationIndex = lastMigration
    ? upMigrationNames.indexOf(lastMigration?.name)
    : -1;
  ...
  const migrationTransaction = database.transaction((name, sql) => {
    ...
    database.run(sql);
    database
      .prepare(`INSERT INTO migrations (name) VALUES ($name)`)
      .run({ $name: name });
